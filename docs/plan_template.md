...
> 本模板为 data_acquisition source、清洗动线与分析场景**共用**的 9 章骨架。类型化完整示例见 `data_acquisition/sources/_template/plan.md.example`（数据获取）、`pipelines/_template/plan.md.example`（清洗）与 `analysis/scenes/_template/plan.md.example`（分析）。
...
- （数据获取 source）source_type / source_handle / 权限边界 / export format：
- 覆盖范围（时间、渠道、指标）：
- 脏数据 / 缺失 / 特殊值情况（如字符串 "(NULL)"、多种日期格式）：
- （分析场景）依赖哪些清洗产物 或 用户直接提供的表：
...
- （数据获取 source）raw_format / raw_filename_pattern / expected_columns / data_acquisition_log 字段：
...
- 数据获取：required_columns、row_count_min、date_range、sample_total_recalc、source evidence 字段完整。
...
- 步骤列表（脚本 + 参数；中间产物用 produces 显式声明）：
- 运行依赖 requires（如 openpyxl / lxml / libreoffice / 环境内 spreadsheet 或 xlsx 能力）：
- （数据获取 source）execution.required_refs / ref_read_order / instruction_policy / source_preflight / fallback：
...
> 本章在注册阶段整理。自迭代不在普通共创/测试时直接改 Skill 本体，而是在 pipeline / analysis scene 注册 gate 做结构化复盘。
