# EXECUTION_SOP —— 执行标准流程（数据清洗 / 数据分析 / 跨数据清洗套件）
...
1. **数据可达性先检**：没有本地 raw data / clean CSV 时，不得直接进入清洗或分析。先按 `docs/DATA_ACQUISITION_SOP.md` 匹配或共创 data_acquisition source，完成 source_preflight、access / runtime requirements 确认、raw data 校验和 `data_acquisition_log.json` 后再 handoff。
...
4. **断点续跑**：每步完成立即通过 `tools/output_manager.py step` 把 `{step名, status}` 写入本次运行的 `info.json` `steps` 区块，并同步生成人读 `data_cleansing_info.yaml` / `data_analysis_info.yaml`。开始执行前先检查输出区是否有同输入的未完成 run（`info.json.steps` 未全 done）→ 有则报告"上次执行到第 N 步（XX）"并问用户"续跑还是重跑？"。
...
8. **旧 run 清理**：同数据清洗套件新 run 通过后，旧 run 只能由 `tools/cleanup_manager.py` 按 `run_lifecycle.cleanup_policy` 生成 plan / 执行清理；默认只列候选并请用户确认，不手工删除。
9. **异常总则**：任何校验不过 → 输出校验报告路径 + 摘要，**不交付产物**；脚本报错 → 报错原文，不猜测原因空转。
10. **数据获取缺支持处理**：data_acquisition source 缺权限、缺后端或缺安全运行支持时，暂停并反馈用户需要补齐的支持项；用户补齐后重新 source_preflight，不直接进入清洗 / 分析。真实 `.env` 不进入 Skill / source / suite 目录，只记录外部引用名。

## 2. 清洗线 checklist

```
□ 0. 优先使用标准 runner
     若是已注册清洗动线复用，优先运行 `tools/data_cleansing_runner.py run` 串联匹配、preflight、
...
□ 1. 匹配数据清洗套件
     读 data_cleansing/manifest.json（只看 status=enabled）。先用 filename_hint/pattern_hint 初筛，
     再读命中套件的 cleansing.yaml 做核验。多命中 → 列出差异请用户选。全不中 → 转共创。
...
□ 2. 理解 + 确认闸门（按 understand.mode 分支）
     [fixed]
       ...
       ├─ 该数据清洗套件首次执行（usage.json 无记录）→ 向用户确认："已匹配 {套件名}，指纹全中，开始清洗？"
       ├─ 非首次 + 全中 → 免确认直接执行，告知一句："已匹配 {套件名}（指纹全中），直接执行。"
...
□ 3. preflight：先确认 raw data 已在本地且通过 data_acquisition source contract 或用户上传文件基本检查；
     ...
     输入文件可读；输出可写；运行 `tools/drift_check.py --suite-dir {cleansing_dir} --input {file}`，
     pass 才可免确认继续，partial 必须点名差异请用户判断，fail 转局部重共创；
     记录本次表格后端、derived recommendation、视觉验收风险和可能的降级策略

□ 4. 建输出目录（output_manager，先占位命名）

□ 5. 变换：按 yaml transform.steps 顺序执行；当前 runner 只替换 `{input}`、`{output}`、`{run_dir}`、`{suite_dir}`、`{skill_root}`。
     多步清洗必须显式写入 run_dir 下的具体文件路径，不得使用 `{mid}` 等未支持 placeholder。
...
□ 7. 校验：按 validation.layers 顺序执行，最终必须生成 validation contract
     ...
     - 增量终检（可选提效）：仅当 输出区存在同数据清洗套件上一次已 verified 的运行
       且 本次指纹/结构一致 且 数据仅为日期顺延追加 时，
       允许"新增日期行全量逐格 + 历史行抽样"；否则全量逐格

□ 8. 写 data_cleansing_info.yaml（含 steps 全清单、data_date_range、validation、outputs、
...
□ 9. 旧 run 生命周期处理（只在 verified / partial_verified 后执行）
     ...
     - 若 cleansing.yaml 未声明 run_lifecycle，则按 cleanup_policy=ask：生成 cleanup_plan.json，列出同数据清洗套件旧 run 候选，请用户确认
     - 若 cleanup_policy=auto_delete_csv，则调用 cleanup_manager apply，只删除旧 run 的 CSV 产物，并保留 info / validation / tombstone
     - pinned 或被 data_analysis 引用的旧 run 默认保护，不删除
...
□ 10. 证据交付（按 validation contract，不得越界承诺）：
     ...
     - 校验报告：{路径}
     - cleanup 状态：{无旧 run / 已生成 cleanup_plan / 已按 policy 清理 / 因 pinned 或 data_analysis 引用而保护}
...
□ 1. 数据分析套件定位（读 data_analysis/manifest.json，只看 status=enabled）
     ├─ 精确命中 keywords → 直接执行
     ├─ 模糊但可意会 → 执行 + 告知"我用了 {数据分析套件名}"
     ├─ 完全不明确 → 列出可用数据分析套件 + 一句话说明，请用户选
     └─ 不匹配 → 转共创

□ 2. 读 analysis.yaml + CALIBERS.md

□ 3. 定位数据（按 inputs 分两类）
     a. data_cleansing 依赖：扫描本 Session 输出区各来源数据清洗套件最新 data_cleansing_info.yaml
        （按 data_date 最新；并列取 run_time 最新；params 可用 --as-of 钉住日期）
        → 某依赖动线无任何产物 → 若有可用 data_acquisition source，先取 raw data 并完成清洗；
          否则告知"本分析依赖 {X} 数据清洗套件的清洗数据，请先上传对应报表或共创数据获取 source"
          → 取数/清洗完成后自动接回本分析请求
...
□ 9. 回填 data_date、写 data_analysis_info.yaml（source_data_cleansing_runs/params/validation）；只有 contract 状态为 verified / partial_verified 才允许 usage.json +1
...
## 5. 跨数据清洗套件分析（inputs.data_cleansing 长度 > 1）
