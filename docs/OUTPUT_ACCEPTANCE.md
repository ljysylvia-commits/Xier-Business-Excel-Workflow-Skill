...
- 产物目录符合 `output/pipelines/{id}_{YYYYMMDD}_{seq}/` 或 `output/analysis/{id}_{YYYYMMDD}_{seq}/`。
- 目录内包含产物文件、`info.json`、对应人读 `pipeline_info.yaml` / `analysis_info.yaml` 视图、执行日志、校验日志。
- `info.json` 是机读事实源，记录输入文件、参数、数据日期范围、steps、可用工具链/降级原因、产物清单和 validation contract 摘要；`pipeline_info.yaml` / `analysis_info.yaml` 只作为同内容的人读视图。
- plan 第 8 章记录本次执行、失败、修复和最终通过状态。
- 向用户交付时必须包含证据摘要：验证状态、验证范围、未验证范围、oracle 来源、假设项、关键数复算和校验报告路径。
- 若同 pipeline 存在旧 run，交付时必须说明 cleanup 状态：无旧 run、已生成 cleanup_plan 待确认、已清理旧 CSV、或因 pinned / analysis 引用而保护。

## 2. 清洗 CSV

- schema 与 plan / pipeline.yaml 一致。
- 日期、数值、比例、货币、空值类型已统一。
...
- 被 analysis run 引用或标记 `pinned: true` 的旧 run 默认不得删除。
- cleanup 后旧 run 目录必须保留 `info.json`、`pipeline_info.yaml`、validation contract / report，并写入 `cleanup_tombstone.json`。
